...
export function registerServiceWorker(onUpdateAvailable) {
    ...
    navigator.serviceWorker.addEventListener('controllerchange', forceReloadOnce);
}

// Chamado quando o usuário clica em "Atualizar agora" no aviso de nova
// versão: manda a versão em espera assumir; o reload acontece via o
// listener de 'controllerchange' configurado acima — e, como rede de
// segurança, também por um timeout curto, caso esse evento não dispare
// (compartilha a mesma flag "reloaded" para nunca recarregar duas vezes).
export function applyUpdate() {
    if (currentRegistration && currentRegistration.waiting) {
        currentRegistration.waiting.postMessage({ type: 'SKIP_WAITING' });
        setTimeout(forceReloadOnce, 2000);
    } else {
